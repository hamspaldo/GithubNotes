---
title: "Using Github with RStudio on Azure Databricks"
...
In RStudio.  
File -> New Project -> Version Control -> Git -> paste the https link from the green repo button into the RStudio "repository URL", _something similar to_:  
...
- It looks like SSH isn't supported by the Azure Databricks instance of RStudio. Have read it may work with SAML single sign-on available through as an Enterprise Github Account - per https://github.com/pricing or google "github subscriptions"  
   
- SSH does work on  cloudera/Taysols, need to create local SSH key on cluster, where the terminal in RStudio is working. (Terminal in RStudio for Azure Databricks is blank, so SSH setup doesn't look to be possible.)  


1. Spin up your RStudio instance.
Configure your git environment in that instance:

usethis::use_git_config(user.name = "hamspaldo", user.email = "hamish.spalding@hotmail.com")

2. create SSH key using Tools -> GLobal Options -> Git/SVN -> create RSA Key -> put in a passphrase for your SSH key -> view public key
copy the public key

3. in R check that you have an SSH key set up.
file.exists("~/.ssh/id_rsa.pub")

4. go to git -> settings
copy and past the SSH key - give it a name like databricksinstance so you know to delete it later

5. In RStudio. File -> New Project -> Version Control -> Git -> paste into repository URL the following:
Git SSH link from the Repo e.g. "git@github.com:hamspaldo/project.git"

6. warning message pops up, say yes

7. enter your passphrase  

## Method 3 using Personal Access Tokens

1. generate PAT in git.  Login ot git -> settings -> developer settings -> personal access tokens -> Generate New token
21254bd4NotaRealPatToken2ebNeverDisclose007c6700  

2. usethis::edit_r_environ() to create .Renviron  
paste in your GITHUB_PAT =21254bd4NotaRealPatToken2ebNeverDisclose007c6700  
save and restart R by 
Session -> restart R  
